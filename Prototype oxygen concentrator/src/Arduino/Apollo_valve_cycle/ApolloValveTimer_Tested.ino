...
//These libraries are not currently needed, further work is to be done on the I2C digital display
//#include <Wire.h>
...
//const int stepsPerRevolution = 2038; //need to test by assuming research is correct that a 28BYJ-48 motor needs 2038 steps per turn
...
//Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET); //defiens the display object 
...
void setup() {
  ...
  Serial.println("Fan on yellow LED");
...
void loop() {
  ...
  int ValveTime5way = map(TimeCtrl5way, 0, 1023, 0, 100); // converts the analogue pot signal which goes from 0 to 1023 bits (1024 bit total) to value of 0 - 100
  ...
  //display.println("5 way valve time RLED" + String(ValveTime5way)); //this goes before the delay function to ensure the time is displayed, could be amended later to count down the time. Prints the time pot value as converted to the oled screen

  Serial.println("5 way valve time RLED:");

  Serial.println((timing_unit_scale_milliseconds * ValveTime5way)/1000); //Print time in seconds
  ...
  Serial.println("5 way valve off RLED");
  ...
  int ValveTime2way = map(TimeCtrl2way, 0, 1023, 0, 100);
  ...
  //display.println("2 way valve time GLED" + Straing (ValveTime2way));

  Serial.println("2 way valve time GLED:");

  Serial.println((timing_unit_scale_milliseconds * ValveTime2way)/1000); //Print time in seconds
  ...
  Serial.println("2 way valve off GLED");
